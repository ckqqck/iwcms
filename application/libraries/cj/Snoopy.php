...
class CI_Snoopy {
	...
	var $maxredirs = 3; // http redirection depth maximum. 0 = disallow
	...
	function fetch($URI) {
		...
		$fp = null;
		$URI_PARTS ["scheme"] = isset ( $URI_PARTS ["scheme"] ) ? $URI_PARTS ["scheme"] : '';
	...
	function _httprequest($url, $fp, $URI, $http_method, $content_type = "", $body = "") {
		...
		if (($this->_framedepth < $this->maxframes) && preg_match_all ( "'<frame\s+.*src[\s]*=[\'\"]?([^\'\"\>]+)'i", $results, $match )) {
			...
			$current_encoding = mb_detect_encoding ( $results, array (
					"ASCII",
					"GB2312",
					"GBK",
					"UTF-8" 
			...
			$this->results [] = iconv ( $current_encoding, $this->unicode, $results );
		} else {
			$current_encoding = mb_detect_encoding ( $results, array (
					"ASCII",
					"GB2312",
			...
			if (isset ( $this->results ) && ! $this->results) {
				$this->results = $results;
			}
		}
		return $this;
