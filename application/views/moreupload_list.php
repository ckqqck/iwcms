...
<body>
	...
	<script>
layui.use(['upload', 'jquery', 'layer','flow'], function(){
  $=layui.jquery,
  layer=layui.layer,
  upload=layui.upload,
  flow = layui.flow;
...
$('#uploaderFiles').on('click', function (e) {
	...
	<div class="layui-layer-btn layui-layer-btn- weui-tabbar">
	<a id="ckss" class="layui-layer-btn0">插入选定图片</a><a id="closeIframe"
			class="layui-layer-btn1">关闭</a>
	</div>
	...
	<script>
		...
		//在父层弹出一个层
		$('#ckss').on('click', function(){
			var list = [];
			var lis = $('#uploaderFiles').children("li");
			for (i = 0; i < lis.length; i++) {
                if ($(lis[i]).hasClass('selected')) {
                    var img = lis[i].firstElementChild,
                        src = img.getAttribute('src'),
                        id = img.getAttribute('data-id');
                    	parent.$('#img').append('<li class="weui_uploader_file weui_uploader_status" id="' + id + '"><div class="ui-grid-halve-img ui-tag-selected"><img src="' + src + '"></div></li>');
                }
            }
